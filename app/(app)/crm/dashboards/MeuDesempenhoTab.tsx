...
import { Goal, KanbanSquare, Handshake, BarChart3, AlertCircle, PhoneCall, Send } from "lucide-react";
...
import { GaugeChart, WeeklyBarChart } from "./DashboardCharts";
...
export async function MeuDesempenhoTab({ agentId, config, userId }: { agentId: string; config: AgentConfig; userId: string }) {
  const now = new Date();
  ...
  const meusEnvios = await prisma.message.findMany({
    where: { role: "human", senderId: userId, conversation: { agentConfigId: config.id } },
  ...
  const conversasQueAbri = Array.from(primeiraPorConversa.values()).filter(m => m.role === "human" && m.senderId === userId);
  ...
  const minhaMeta = metasPorVendedor?.[userId] ?? 0;
