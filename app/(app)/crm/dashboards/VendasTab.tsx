...
export async function VendasTab({ agentId, config, from, to }: {
  ...
  for (const o of wonAllTime) {
    ...
    const clientMap2 = ltvPorVendedorAcc.get(vendedorId) ?? new Map<string, number>();
    clientMap2.set(o.conversation.contactNumber, (clientMap2.get(o.conversation.contactNumber) ?? 0) + o.dealValue);
    ltvPorVendedorAcc.set(vendedorId, clientMap2);
  }
  const ltvPorVendedor = Array.from(ltvPorVendedorAcc.entries())
    .map(([id, clientMap2]) => ({
      id,
      name: id === "__sem__" ? "Sem atendente" : (nameById.get(id) ?? "Ex-membro"),
      ltv: Array.from(clientMap2.values()).reduce((s, v) => s + v, 0) / clientMap2.size,
      clientes: clientMap2.size,
    }))
