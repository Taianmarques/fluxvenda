...
// Corpo da conversa (cabeçalho opcional + mensagens + envio) — extraído de ConversationPopup
// pra ser reaproveitado tanto pelo popup (com casca de modal própria) quanto pela aba
// "Conversa" do modal de detalhes da oportunidade (sem casca, já dentro de outro modal).
export function ConversationThread({ conversationId, dark, onClose, showHeader = true }: {
  ...
  return (
    <div className={`h-full flex flex-col overflow-hidden ${dark ? "text-white" : "text-slate-900"}`}>
      {showHeader && (
        <div className={`px-4 py-3 border-b flex items-center justify-between flex-shrink-0 ${dark ? "border-gray-800" : "border-gray-200"}`}>
          <div>
            ...
            {detail?.contactName && <p className="text-xs text-gray-500">{detail.contactNumber}</p>}
          </div>
          {onClose && (
              ...
              <div key={m.id}>
                ...
                <div
                  className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${
                    m.role === "human" ? (dark ? "bg-green-700 text-white" : "bg-[#cfe9ff] text-slate-900") :
                    m.role === "assistant" ? (dark ? "bg-[#005c4b] text-white" : "bg-[#d9fdd3] text-slate-900") :
                    dark ? "bg-[#202c33] text-gray-100" : "bg-white text-slate-900"
                  }`}
                >
                  {m.forwarded && (
