...
export default function AgendamentosPage(props: { params: Promise<{ agentId: string }> }) {
  return (
    <CrmPageGate pageKey="agendamentos">
      <AgendamentosPageContent {...props} />
    </CrmPageGate>
  );
}

async function AgendamentosPageContent({ params }: { params: Promise<{ agentId: string }> }) {
  ...
  if (!config?.active || !config.schedulingEnabled) {
    return (
      <div className="h-full bg-gray-950 text-white p-6 flex items-center justify-center">
        <div className="max-w-md text-center space-y-4">
          <CalendarCheck size={48} className="mx-auto text-blue-400" />
          <h1 className="text-2xl font-bold">Agendamentos não configurados</h1>
          <p className="text-gray-400">Ative o agendamento na página Agenda para começar a receber marcações e ver os dados aqui.</p>
    ...
    <div className="h-full overflow-y-auto bg-gray-950 text-white p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <h1 className="text-3xl font-bold mt-1 flex items-center gap-2"><CalendarCheck size={28} className="text-blue-400" /> Agendamentos</h1>
          <p className="text-gray-400 mt-1">Volume, status e desempenho dos agendamentos feitos pelo agente.</p>
        </div>
