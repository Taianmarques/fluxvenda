...
async function VendasPageContent({ params }: { params: Promise<{ agentId: string }> }) {
  ...
  const wonDeals = await prisma.opportunity.findMany({
    where: { wonAt: { not: null }, conversation: { agentConfigId: config.id, isSandbox: false, isTestNumber: false, isGroup: false } },
    orderBy: { wonAt: "desc" },
  ...
  const openDeals = await prisma.opportunity.findMany({
    where: { wonAt: null, conversation: { agentConfigId: config.id, isSandbox: false, isTestNumber: false, isGroup: false } },
    include: { stage: true },
  });
  ...
  const encerradas = await prisma.conversation.findMany({
    where: { agentConfigId: config.id, status: "FINALIZADO", motivoEncerramento: { not: null }, isSandbox: false, isTestNumber: false, isGroup: false },
