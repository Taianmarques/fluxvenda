...
async function VendasPageContent({ params }: { params: Promise<{ agentId: string }> }) {
    ...
    return (
      <div className="h-full bg-gray-950 p-6 flex items-center justify-center">
        <div className="max-w-md text-center space-y-4">
          ...
          <p className="text-gray-400">Conecte o WhatsApp ou Instagram do seu agente para acompanhar as vendas aqui.</p>
  ...
  const wonDeals = await prisma.opportunity.findMany({
    where: { wonAt: { not: null }, conversation: { agentConfigId: config.id, isSandbox: false, isGroup: false } },
    orderBy: { wonAt: "desc" },
  ...
  const openDeals = await prisma.opportunity.findMany({
    where: { wonAt: null, conversation: { agentConfigId: config.id, isSandbox: false, isGroup: false } },
    include: { stage: true },
  });
  ...
  const encerradas = await prisma.conversation.findMany({
    where: { agentConfigId: config.id, status: "FINALIZADO", motivoEncerramento: { not: null }, isSandbox: false, isGroup: false },
