...
const schema = z.object({
  ...
  horarioEnvioInicio: z.string().regex(/^\d{2}:\d{2}$/).optional(),
  horarioEnvioFim: z.string().regex(/^\d{2}:\d{2}$/).optional(),
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  const config = await prisma.agentConfig.update({
    ...
    data: {
      ...
      ...(d.horarioEnvioInicio !== undefined && { horarioEnvioInicio: d.horarioEnvioInicio }),
      ...(d.horarioEnvioFim !== undefined && { horarioEnvioFim: d.horarioEnvioFim }),
