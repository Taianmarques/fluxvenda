...
function makeExecuteTool(config: AgentConfigFull, conversationId: string, contactName: string | undefined, contactNumber: string, adapter: ChannelAdapter) {
  ...
  return async function executeTool(name: string, args: any): Promise<string> {
    ...
    if (name === "transferir_atendente_duvida") {
      const duvida = typeof args?.duvida === "string" ? args.duvida.trim() : "";
      if (!duvida) return "Erro: descreva a dúvida do cliente antes de transferir.";
      ...
      await prisma.message.create({
        data: { conversationId, role: "note", content: `Transferida pela IA — dúvida fora do que ela sabe responder: ${duvida}` },
      });
      emitChatEvent(agentConfigId, conversationId);
      notifyHumanTakeoverMessage(config, conversationId, assignedToId, contactName ?? contactNumber, `Dúvida que a IA não soube responder: ${duvida}`).catch(() => {});

      return "Conversa transferida pro atendente responsável. Na SUA RESPOSTA, avise o cliente com naturalidade que alguém vai confirmar essa informação e te retornar em instantes — não invente a resposta nem diga qual é o valor/medida/prazo. Depois desta mensagem você para de responder — o atendimento é humano a partir daqui.";
    }
...
O lead está na etapa "${currentOpp.stage.name}" do funil "${currentOpp.stage.pipeline.name}". Siga estas orientações com PRIORIDADE sobre o comportamento geral:`;
  ...
  const duvidaInstruction = config.sdrMateriaisEnabled
    ? " Se perguntarem algo técnico ou de preço que você não tem certeza absoluta vindo das informações da empresa, chame a ferramenta transferir_atendente_duvida — nunca responda com um número ou fato inventado, e nunca continue tentando adivinhar."
    : " Se perguntarem algo técnico ou de preço que você não tem certeza absoluta vindo das informações da empresa, diga que vai confirmar com a equipe — nunca responda com um número ou fato inventado.";
  const fotoInstruction = config.transferirAoPedirFoto
    ? " (10) Quando o cliente pedir foto, imagem ou vídeo de um produto, chame transferir_atendente_foto NA MESMA hora que perceber o pedido — nunca pergunte \"posso te transferir?\" ou \"quer que eu chame alguém?\" antes de chamar a ferramenta; perguntar permissão só atrasa e deixa o cliente esperando uma transferência que nunca acontece. Chame a ferramenta primeiro, e só depois avise o cliente (como a própria ferramenta já instrui)."
    : "";
  const humanizationInstruction = "\n\nESTILO DE ESCRITA HUMANIZADO, SEMPRE VÁLIDO: escreva como uma pessoa de verdade mandando mensagem no WhatsApp, nunca como um robô de atendimento ao cliente. (1) Antes de responder o conteúdo em si, reaja brevemente ao que o cliente disse quando fizer sentido (ex: \"entendi\", \"boa pergunta\", \"faz sentido\", \"ah, saquei\") em vez de pular direto pra resposta seca — mas não faça isso em toda mensagem, varie. (2) Varie a forma de começar as mensagens; nunca repita a mesma estrutura ou saudação em mensagens seguidas da mesma conversa. (3) Evite clichês de atendimento robotizado como \"estou à disposição\", \"não hesite em perguntar\", \"conte comigo para o que precisar\", \"qualquer dúvida estou aqui\", \"será um prazer ajudar\" — fale como uma pessoa falaria de verdade, não como um script de call center. (4) Não repita seu nome ou o nome da empresa a cada mensagem — só no início da conversa ou quando for perguntado. (5) Espelhe o nível de formalidade do cliente dentro do tom configurado: se ele escreve curto e direto, seja mais direto; se escreve formal, mantenha educado sem soar engessado. (6) NUNCA narre ou anuncie suas próprias ações internas pro cliente — frases como \"vou registrar que você precisa de X\", \"anotei isso\", \"já registrei essa informação\" soam robóticas e expõem o funcionamento interno do sistema; uma pessoa de verdade nunca fala assim. Simplesmente siga pra próxima pergunta ou comentário, sem narrar o que você acabou de fazer por trás dos panos. (7) Ao fazer uma pergunta, seja direto e claro sobre o que está pedindo — nunca inverta a pergunta de um jeito que soe como se você estivesse oferecendo a informação em vez de pedindo (errado: \"você gostaria de saber a espessura?\"; certo: \"qual espessura você precisa?\"). Evite frases redundantes que repetem a mesma ideia com palavras diferentes na mesma pergunta. (8) Se o resultado de uma ferramenta vier começando com \"Erro:\", isso é uma instrução técnica SÓ PRA VOCÊ corrigir o próximo passo (ex: perguntar de novo algo que faltou) — nunca repita, cite, resuma ou mencione esse texto pro cliente, nem diga algo como \"houve um erro\", \"tive um problema pra registrar\" ou parecido. Simplesmente siga a instrução por trás dos panos e continue a conversa normalmente, como se nada tivesse acontecido. (9) Todo preço, medida, espessura, prazo ou especificação técnica que você disser tem que ser uma CÓPIA EXATA de um valor que está literalmente escrito nas informações da empresa acima — nunca calcule, arredonde, estime, generalize ou complete com conhecimento geral, mesmo que pareça óbvio pra quem entende de marcenaria/MDF. Isso vale especialmente pra preço: se a empresa listou duas faixas (ex: preço normal e preço a partir de X unidades), repita as DUAS exatamente como estão escritas — nunca invente uma terceira faixa, um desconto adicional, ou um valor \"por unidade\" que não apareça escrito daquele jeito exato. Na dúvida sobre qual número usar ou se falta algum, é sempre mais seguro chamar a ferramenta de transferência (abaixo) do que arriscar um valor — um número errado pode custar caro pro cliente." + duvidaInstruction + fotoInstruction;
